        ...
        for pstate2j in [0.5, 1.5]:
            for pstate1mj in [0.5, 1.5]:
                for pstate2mj in [0.5, 1.5]:
                    dme = atom.getDipoleMatrixElement(np, 0, 0.5, 0.5, pstate1n, 1, pstate1j, pstate1mj,
                                                      pstate1mj - 0.5)
                    dme1 = atom.getDipoleMatrixElement(ni, 0, 0.5, 0.5, pstate2n, 1, pstate2j, pstate2mj,
                                                       pstate2mj - 0.5)
                    ...
                    print("C_3 [%.0f S,%.0f S] = %.3f GHz (mu m)^3\t%.3f mu m" % (
                    ni, np, abs(c3) / C_h * 1.e9, c3blockade))
